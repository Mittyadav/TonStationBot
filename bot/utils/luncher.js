  ...
  async #run_tasks(tgClients) {
    ...
    const tasks = tgClients.map(async (tgClient, index) => {
      ...
      try {
        ...
        logger.info(
          `<ye>[tonstation]</ye> | ${tgClient.session_name} | Sleeping ${sleeping} seconds before starting the bot`
        );
  ...
  async #run_tasks_query() {
    ...
    const tasks = queries?.map(async ([query_name, query_id], index) => {
      ...
      try {
        ...
        logger.info(
          `<ye>[tonstation]</ye> | ${query_name} | Sleeping ${sleeping} seconds before starting the bot`
        );
        await sleep(sleeping);
